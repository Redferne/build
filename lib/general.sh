...
prepare_host()
{
	...
	if [[ -z $codename || "bionic buster eoan focal debbie tricia kali-rolling" != *"$codename"* ]]; then
	...
	if [[ -z $codename || "focal" == "$codename" || "eoan" == "$codename"  || "debbie" == "$codename"  || "buster" == "$codename" || "kali-rolling" == "$codename" ]]; then
